...
import AllPostsList from '../PostsAndStories/AllPostsList';
import '../../Style/AllCampaigns/CampaignDetails.css';
...
const CampaignDetails = ({ itemsPerPage, totalItems, paginate, currentPage }) => {
      const pageNumbers = [];
        ...
        const Pagintation = () => {
          ...
          const [filteredResults, setFilteredResults] = useState(AllPostsList);
  
          const handleSearch = (event) => {
              const searchText = event.target.value;
              setSearchValue(searchText);
              let results = AllPostsList;
              if (searchText) {
              results = AllPostsList.filter((campaign) => campaign.name.toLowerCase().includes(searchText.toLowerCase()));
              }
           ...
           <Container className="mt-2" style={{border:"1px solid rgb(198, 198, 198)"}}>
              <div className='d-lg-flex d-sm-block'>
                  <Col  xs={12} sm={12} md={2} lg={2} className="mt-5" > 
                    ...
                    <p style={{fontSize:"12px"}}>Posted on: {AllCampaignsList[0].startDate}</p>
