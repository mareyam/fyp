...
const Campaigns = () => {
 ...
 //get subreddit name from localhost, show given likes against subreddit names

  //  useEffect(() => {
  //   axios.get('http://127.0.0.1:8000/activecampaigns/')
  //     .then(response => {
  //       setCampaigns(response.data);
  //       setSubreddits(response.hashtag)
  //       console.log("sub is " + campaigns.hashtag);
  //     })
  //     .catch(error => {
  //       console.error(error);
  //     });
  ...
  useEffect(() => {
    axios.get('http://127.0.0.1:8000/activecampaigns/')
      .then(response => {
        setCampaigns(response.data);
        setSubreddits(response.data.map(campaign => campaign.hashtag));
    ...
    const fetchData = async () => {
      try {
        ...
        const postsArray = jsonData
          .filter(post => post.data.subreddit === subreddits && post.data.likes !== null)
          .reduce((acc, post) => acc + post.data.likes, 0);
          setPosts(postsArray);
          console.log(`Subreddit "${subreddits}" has ${postsArray} total likes.`);
